...
import userEvent from '@testing-library/user-event'; // Import userEvent if not already
...
test("the page shows information the user types into the name and email address form fields", () => {
  ...
  expect(message).toHaveTextContent('john.doe@example.com');
});
...
test("a message is displayed when the user clicks the Submit button", () => {
  ...
  userEvent.type(emailInput, 'jane.smith@example.com');
  userEvent.click(checkboxes[0]);
  userEvent.click(submitButton);

  const message = screen.getByText(/form submitted successfully/i);
  expect(message).toBeInTheDocument();
  expect(message).toHaveTextContent('Jane Smith');
  expect(message).toHaveTextContent('jane.smith@example.com');
  expect(message).toHaveTextContent('Interests: Interest1');
});
